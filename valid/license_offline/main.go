...
func init() {
	...
	// define new queue for save random number and check existence
	qu = NewQueue()
}
...
func check(c *fiber.Ctx) error {
	...
	// check random number
	if qu.Exists(cb.RandomNum) {
		return c.Status(400).JSON(fiber.Map{"error": "The random number already exists"})
	}

	// store random number
	qu.Enqueue(cb.RandomNum)
